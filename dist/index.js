...
const plugin = (moduleName = "external", sourceMaps = true) => {
    const replace = typeof moduleName === "string"
                            ...
                            enter(node) {
                                if (importNodeTypes.includes(node.type)) {
                                    ...
                                    if (req && req.value.includes("node_modules")) {
                                        ...
                                        const bundlePath = replace(path.posix.join(path.posix.dirname(fileName), req.value));
                                        // and then format the path relative to the updated chunk path
                                        const newPath = path.posix.relative(path.posix.dirname(chunkInfo.fileName), bundlePath);
                                        // add ./ to files relative to project root
                                        const normalizedPath = newPath.startsWith("./") || newPath.startsWith("../")
                                            ? newPath
                                            : `./${newPath}`;
